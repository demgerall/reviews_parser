...
def set_styles_to_sheet(sheet: Worksheet, num: int) -> None:
    sheet.auto_filter.ref = f"A1:D{num - 1}"
    ...
    for col in ["A", "B", "C", "D"]:
        for row in range(1, num):
            ...
            if col == "C" and row != 1:
                ...
                sheet[f"{col}{row}"].font = Font(color=colors[int(sheet.cell(row=row, column=3).value) - 1])
            if sheet.cell(row=row, column=4).value == "None":
                sheet[f"D{row}"].font = Font(name="Calibri", size=12, bold=True, color="A94123")

    sheet.column_dimensions["A"].width = 25
    sheet.column_dimensions["B"].width = 31
    sheet.column_dimensions["C"].width = 12
    sheet.column_dimensions["D"].width = 180
...
class App(QtWidgets.QMainWindow, designMain.Ui_MainWindow):
    ...
    def search_on_site(self, book: Workbook, site: str) -> None:
        try:
            ...
            sheet.cell(row=1, column=1).value = "Никнейм"
            sheet.cell(row=1, column=2).value = "Дата"
            sheet.cell(row=1, column=3).value = "Оценка"
            sheet.cell(row=1, column=4).value = "Текст отзыва"
            num = 2
    ...
    def get_reviews_elements(self, site: str, num: int, sheet: Worksheet, driver: WebDriver) -> int:
        try:
            driver.get(url=self.config[site]["check_url"])
            ...
            while True:
                if site == "Google" and len(driver.find_elements(By.CSS_SELECTOR, self.config[site]["show_more_button"])) > 0:
    ...
    def get_reviews_data(self, driver: WebDriver, site: str, num: int, sheet: Worksheet,
        ...
        for el in html_els:
            if check_exists(el, self.config[site]["review_name_css_selector"]):
                sheet.cell(row=num, column=1).value = driver.execute_script('return arguments[0].textContent;',
                                                                            el.find_element(By.CSS_SELECTOR,
                                                                                            self.config[site][
                                                                                                "review_name_css_selector"]))  # name
            else:
                sheet.cell(row=num, column=1).value = "None"
            ...
            if check_exists(el, self.config[site]["review_rate_css_selector"]):
                sheet.cell(row=num, column=3).value = len(
            ...
            if check_exists(el, self.config[site]["review_text_css_selector"]):
                sheet.cell(row=num, column=4).value = driver.execute_script('return arguments[0].textContent;',
                                                                            el.find_element(By.CSS_SELECTOR,
                                                                                            self.config[site][
